...
fs.readFile("eleven/eleven.input", "utf8", (err, data) => {
    ...
    var justPainted = _.map(
        _.filter(
            _.map(painted, (x, i) => [x, i]),
            ([x, i]) => x == 1
        ),
        ([x, i]) => manySepBy(",")(int()).parse(i).value
    );

    var l1 = _.repeat(" ", 40);
    _.map(
        _.filter(justPainted, ([w, y]) => y == 0),
        ([x, v]) => {
            l1 = l1.replaceAt(x, "█");
        }
    );
